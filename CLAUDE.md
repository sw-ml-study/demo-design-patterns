# CLAUDE.md — design-patterns-extraction
...
- Prefer thin `just` recipes and do not add a Makefile without an explicit
  external need. Use `$MLPL` or the adjacent release binary; never modify
  `../sw-mlpl` or overwrite stable tools.
- Preserve runnable/constrained/closed/gated distinctions and explicit
  non-claims. Algorithms and data structures belong in their sibling repos.
